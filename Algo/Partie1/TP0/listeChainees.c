...
cel * nouvCell(point p){
    ...
    if(!tmp){
        ...
        tmp->p = p;
        //on initialise la cellule suivante et precedente  null
        tmp->pre = tmp->succ = NULL;
        return tmp;
    }
}
//indexation commence a 1
//fonction pour inserer une cellule dans la liste : on insere apres la position pos
void insererCel(cel *liste,int pos, cel *cellule){
    cel *posListe = liste;
    int k;
    //on parcourt laliste juaqu'a ce que k soit egal a la position demande puis on insere la nouvelle cellule apres la cellule courante
...
//fonction pour supprimer la cellule a la position pos
...
//fonciton qui menu qui permet la saisie de laliste par l'utilisateur  
